import { handleToolCall, resolveVisibleJobByName, resolveVisibleTempByName, } from "./toolHandlers.js";
import { buildAssignPendingAction, buildClarificationPromptsForJobs, buildClarificationPromptsForTemps, buildResolvedEntities, buildUnassignPendingAction, extractReplyTextFromToolResult, textReply, } from "./chatActions.js";
import { extractJobId, extractTempId, isAssignIntent, isAvailabilityExplanationIntent, isAvailableTempsIntent, isBestTempIntent, isJobDetailsIntent, isTempDetailsIntent, isUnassignIntent, normalizeText, } from "./chatText.js";
export async function handleChatMessage(message, context) {
    ...
    if (confirmAssignMatch) {
        ...
        const result = await handleToolCall("assign_temp_to_job", { jobId, tempId }, context);
        return {
            status: result.status,
            body: {
                reply: extractReplyTextFromToolResult(result),
                suggestedActions: result.status === 200
                    ? [
                        {
                            type: "send_message",
    ...
    if (confirmUnassignMatch) {
        ...
        const result = await handleToolCall("unassign_temp_from_job", { jobId }, context);
        return {
            status: result.status,
            body: {
                reply: extractReplyTextFromToolResult(result),
    ...
    if (resolvedJobId == null && currentJobId != null) {
        const jobIntentWithoutExplicitJob = isAssignIntent(normalized) ||
    ...
    if (isAssignIntent(normalized)) {
        ...
        if (resolvedTempId == null) {
            return {
                status: 200,
                body: {
                    reply: "I need a temp before I can assign anyone. Include a temp ID, say 'Assign them' after I recommend a temp, or mention the person by name.",
                    suggestedActions: [
                        {
                            type: "send_message",
                            label: "Show available temps",
    ...
    if (isAvailableTempsIntent(normalized) && resolvedJobId != null) {
        const result = await handleToolCall("get_available_temps_for_job", { jobId: resolvedJobId }, context);
        return {
            status: result.status,
            body: {
                reply: extractReplyTextFromToolResult(result),
    ...
    if (isBestTempIntent(normalized) && resolvedJobId != null) {
        const result = await handleToolCall("suggest_best_temp_for_job", { jobId: resolvedJobId }, context);
    ...
    if (isAvailabilityExplanationIntent(normalized) && resolvedJobId != null && resolvedTempId != null) {
        const result = await handleToolCall("explain_temp_availability_for_job", { jobId: resolvedJobId, tempId: resolvedTempId }, context);
        return {
            status: result.status,
            body: {
                reply: extractReplyTextFromToolResult(result),
    ...
    if (isAvailabilityExplanationIntent(normalized)) {
        return {
            status: 200,
            body: {
                reply: "I need both a temp and a job to explain availability. Ask after a temp suggestion, or include both IDs.",
                suggestedActions: [],
                clarificationPrompts: [
                    {
                        id: "why-temp-5",
    ...
    return textReply(currentJobId != null
